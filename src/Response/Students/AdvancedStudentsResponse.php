<?php

namespace PHPinnacle\Langcat\Response\Students;

use PHPinnacle\Langcat\Response\Shared\PaginationResponse;
use PHPinnacle\Langcat\Support\ResponseValue;
use UnexpectedValueException;

final readonly class AdvancedStudentsResponse
{
    /** @param list<AdvancedStudentResponse> $data */
    public function __construct(
        public array $data,
        public PaginationResponse $meta,
    ) {}

    /** @param array<array-key, mixed> $payload */
    public static function fromArray(array $payload): self
    {
        return new self(
            array_map(
                static fn (mixed $student) => AdvancedStudentResponse::fromArray(
                    is_array($student)
                        ? $student
                        : throw new UnexpectedValueException('Each student must be an object.'),
                ),
                ResponseValue::list($payload, 'data'),
            ),
            PaginationResponse::fromArray(ResponseValue::object($payload, 'meta')),
        );
    }
}
